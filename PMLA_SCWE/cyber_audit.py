"""Cyber wellness audit module for PMLA-SCWE."""

from __future__ import annotations
from datetime import date
from .database import execute_select, execute_non_query


def calculate_wellness_score(audit_data: dict) -> float:
    """Calculates a student's wellness index score (0-100).
    
    Based on:
    - Sleep duration (25%): optimal 7-9 hours.
    - Recreational screen time (25%): optimal <= 2 hours.
    - Digital distraction level (25%): 1-5 scale (lower is better).
    - Cyber safety awareness (25%): 1-5 scale (higher is better).
    """
    # 1. Sleep score
    try:
...
def validate_audit_data(audit_data: dict) -> list[str]:
    ...
    if "student_id" not in audit_data:
        ...
        try:
            ...
            res = execute_select("SELECT COUNT(*) AS cnt FROM Students WHERE student_id = %s", (student_id,))
...
def add_cyber_audit(audit_data: dict) -> bool:
    ...
    query = """
        INSERT INTO Cyber_Audit (
            student_id, daily_screen_time, study_screen_time, recreational_screen_time,
            sleep_duration, digital_distraction_level, cyber_safety_awareness,
            screen_time_hours, wellness_score, remarks, audit_date
        ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
    """
    params = (
        int(audit_data["student_id"]),
        float(audit_data["daily_screen_time"]),
        ...
        float(audit_data["daily_screen_time"]),  # for screen_time_hours compatibility
        wellness_score,
        audit_data.get("remarks") or "",
        audit_data.get("audit_date") or date.today().isoformat()
    )

    try:
        affected = execute_non_query(query, params)
        return affected > 0
    except Exception:
        return False


def get_student_audits(student_id: int) -> list[dict]:
    """Retrieves all cyber-wellness audit records for a student."""
    query = """
        SELECT audit_id, student_id, daily_screen_time, study_screen_time, recreational_screen_time,
               sleep_duration, digital_distraction_level, cyber_safety_awareness,
    ...
    """
    try:
        return execute_select(query, (student_id,))
    except Exception:
        return []


def get_latest_audit(student_id: int) -> dict | None:
    """Retrieves the most recent cyber-wellness audit record for a student."""
    query = """
        SELECT audit_id, student_id, daily_screen_time, study_screen_time, recreational_screen_time,
               sleep_duration, digital_distraction_level, cyber_safety_awareness,
               wellness_score, audit_date, remarks
    ...
    """
    try:
        rows = execute_select(query, (student_id,))
